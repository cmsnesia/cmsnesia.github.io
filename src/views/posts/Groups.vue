    ...
    <b-card>
      <div slot="header" show-header>
        <b-navbar toggleable="lg">
          <b-navbar-brand href="#">Category Group</b-navbar-brand>
      ...
      <b-modal ref="categoryGroupModal" title="Tambah Menu">
        <div class="form-row">
          <div class="col-12">
            <ValidationObserver ref="observer" v-slot="{ invalid }">
              <div class="form-group">
                ...
                <ValidationProvider rules="required|alpha_spaces" v-slot="{ errors }">
                  <input type="text" class="form-control" v-model="categoryGroupName" required>
                  <span>{{ errors[0] }}</span>
                </ValidationProvider>
                <br/>
                ...
                <ValidationProvider v-slot="{ errors }">
                  <multiselect style="height: 40px" v-model="selectedCategories" id="ajax" label="name" track-by="id"
                    ...
                    <span slot="noResult">Kategori tidak ditemukan. Selahkan menggunakan kata kunci lain.</span>
                  </multiselect>
                </ValidationProvider>
              </div>
            </ValidationObserver>
...
const categoryGroupService = CategoryGroupService.build()
...
export default {
  ...
  methods: {
    getDataTable: async function () {
      ...
      let data = {}
      const response = await categoryGroupService.find(data, param)
    ...
    async doSave () {
      ...
      if (this.id) {
        ...
        categoryGroupService.edit(data).then(result => {
          swall('Data berhasil disimpan!', {
            icon: 'success',
            title: 'Sukses',
            text: result.statusCode.message
          })
          this.$refs['categoryGroupModal'].hide()
          this.resetFields()
          this.getDataTable()
        })
      } else {
        ...
        categoryGroupService.add(data).then(result => {
          swall('Data berhasil disimpan!', {
            icon: 'success',
            title: 'Sukses',
            text: result.statusCode.message
    ...
    async doDelete (val) {
      ...
      categoryGroupService.delete(data).then(result => {
    ...
    async onClickButtonAdd () {
      ...
      this.$refs['categoryGroupModal'].show()
    },
    async onClickButtonEdit (val) {
      this.id = val.id
      if (val.id) {
        this.$refs['categoryGroupModal'].show()
        this.categoryGroupName = val.name
        const data = val.categoryIds.map(id => {
          return Object.assign({ 'id': id })
        })
        const response = await categoryService.findByIds(data)
